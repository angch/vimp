...
pub const WelcomeScreen = struct {
    // Callback definition must be before usage in fields if referencing it, or just ordered cleanly.
    // In Zig structs, declarations (pub const) are usually at top or bottom.
    // The error "declarations are not allowed between container fields" means we cannot interleave fields and decls.
    ...
    fn on_child_activated(_: *c.GtkFlowBox, child: *c.GtkFlowBoxChild, user_data: ?*anyopaque) callconv(std.builtin.CallingConvention.c) void {
        ...
        if (data) |p| {
            const path_c: [*:0]const u8 = @ptrCast(p);
            const len = std.mem.len(path_c);
            const path_z = path_c[0..len :0];

            // Invoke callback
            self.open_callback(self.open_callback_data, path_z);
        }
    }
