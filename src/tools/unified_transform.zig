...
pub const UnifiedTransformTool = struct {
    allocator: std.mem.Allocator,

    pub fn create(allocator: std.mem.Allocator) !*UnifiedTransformTool {
        const self = try allocator.create(UnifiedTransformTool);
        self.* = .{
            .allocator = allocator,
        };
        return self;
    ...
    pub fn interface(self: *UnifiedTransformTool) ToolInterface {
    ...
    fn destroy(ptr: *anyopaque, allocator: std.mem.Allocator) void {
        const self: *UnifiedTransformTool = @ptrCast(@alignCast(ptr));
