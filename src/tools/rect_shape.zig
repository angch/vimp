...
pub const RectShapeTool = struct {
    allocator: std.mem.Allocator,
    start_x: f64,
    start_y: f64,

    pub fn create(allocator: std.mem.Allocator) !*RectShapeTool {
        const self = try allocator.create(RectShapeTool);
        self.* = .{
            .allocator = allocator,
            .start_x = 0,
            .start_y = 0,
    ...
    pub fn interface(self: *RectShapeTool) ToolInterface {
    ...
    fn start(ptr: *anyopaque, engine: *Engine, x: f64, y: f64, button: u32, modifiers: u32) void {
        const self: *RectShapeTool = @ptrCast(@alignCast(ptr));
        _ = modifiers;
        if (button != 1) return;
        self.start_x = x;
        self.start_y = y;
    ...
    fn update(ptr: *anyopaque, engine: *Engine, x: f64, y: f64, modifiers: u32) void {
        ...
        if (engine.preview_shape) |*s| s.type = .rectangle;
    }
