...
pub const PencilTool = struct {
    allocator: std.mem.Allocator,
    last_x: f64,
    last_y: f64,
    active_button: u32,

    pub fn create(allocator: std.mem.Allocator) !*PencilTool {
        const self = try allocator.create(PencilTool);
        self.* = .{
            .allocator = allocator,
            .last_x = 0,
            .last_y = 0,
    ...
    pub fn interface(self: *PencilTool) ToolInterface {
    ...
    fn start(ptr: *anyopaque, engine: *Engine, x: f64, y: f64, button: u32, modifiers: u32) void {
        const self: *PencilTool = @ptrCast(@alignCast(ptr));
        _ = modifiers;
        self.last_x = x;
        self.last_y = y;
