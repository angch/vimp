...
pub const BrushTool = struct {
    ...
    pub fn create(allocator: std.mem.Allocator) !*BrushTool {
        const self = try allocator.create(BrushTool);
        self.* = .{
            .allocator = allocator,
            .last_x = 0,
            .last_y = 0,
    ...
    pub fn interface(self: *BrushTool) ToolInterface {
    ...
    fn start(ptr: *anyopaque, engine: *Engine, x: f64, y: f64, button: u32, modifiers: u32) void {
        const self: *BrushTool = @ptrCast(@alignCast(ptr));
        _ = modifiers;
        self.last_x = x;
        self.last_y = y;
        ...
        if (button == 3) { // Right click -> BG color
