...
pub const EllipseShapeTool = struct {
    allocator: std.mem.Allocator,
    start_x: f64,
    start_y: f64,

    pub fn create(allocator: std.mem.Allocator) !*EllipseShapeTool {
        const self = try allocator.create(EllipseShapeTool);
        self.* = .{
            .allocator = allocator,
            .start_x = 0,
            .start_y = 0,
    ...
    pub fn interface(self: *EllipseShapeTool) ToolInterface {
    ...
    fn start(ptr: *anyopaque, engine: *Engine, x: f64, y: f64, button: u32, modifiers: u32) void {
        const self: *EllipseShapeTool = @ptrCast(@alignCast(ptr));
        _ = modifiers;
        if (button != 1) return;
        self.start_x = x;
        self.start_y = y;
