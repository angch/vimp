...
pub const EllipseSelectTool = struct {
    allocator: std.mem.Allocator,
    start_x: f64,
    start_y: f64,
    is_moving: bool,

    pub fn create(allocator: std.mem.Allocator) !*EllipseSelectTool {
        const self = try allocator.create(EllipseSelectTool);
        self.* = .{
            .allocator = allocator,
            .start_x = 0,
            .start_y = 0,
    ...
    pub fn interface(self: *EllipseSelectTool) ToolInterface {
    ...
    fn start(ptr: *anyopaque, engine: *Engine, x: f64, y: f64, button: u32, modifiers: u32) void {
        const self: *EllipseSelectTool = @ptrCast(@alignCast(ptr));
        _ = modifiers;
        if (button != 1) return;
