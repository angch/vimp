...
- **XCF Parsing**: `PROP_VECTORS` (25) is used for modern paths (Bezier strokes), while `PROP_PATHS` (23) is for legacy paths. Points in `PROP_VECTORS` are stored as a flat list of coordinates, interpretable as a standard Bezier sequence (Anchor, Control, Control, Anchor...). `gegl_path_parse_string` handles standard SVG path strings (M, C, Z).
- **UI Refactoring**: `src/main.zig` has been decomposed. The canvas logic is now in `src/ui/canvas.zig`. When refactoring C-style callbacks (e.g. `draw_func`), pass the struct instance pointer as `user_data` and cast it back (`@ptrCast(@alignCast(user_data))`) within the callback to access state.
