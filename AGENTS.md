...
- **Tool State Management**: Tools like `BrushTool` and `PencilTool` store state (like `last_x`, `last_y`, `active_button`) within their struct. The `interface()` method returns a wrapper exposing function pointers. Care must be taken to ensure the tool pointer (`ptr`) is correctly cast back to the concrete type in callbacks. Memory management is handled by `main.zig` calling `tool.destroy()` before switching.
